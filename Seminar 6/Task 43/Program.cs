Console.WriteLine("Введи k1 и b1 через пробел");
decimal x;
decimal y1, y2;
string[] kb = Console.ReadLine().Split(" ");
decimal k1 = Convert.ToDecimal(kb[0]);
decimal b1 = Convert.ToDecimal(kb[1]);
Console.WriteLine("Введи k2 и b2 через пробел");
kb = Console.ReadLine().Split(" ");
decimal k2 = Convert.ToDecimal(kb[0]);
decimal b2 = Convert.ToDecimal(kb[1]);
x = (b2 - b1) / (k1 - k2);
y1 = k1 * x + b1;
y2 = k2 * x + b2;
Console.WriteLine("Координаты точки пересечения x = " + x + "  y1 = " + y1 + "  y2 = " + y2);